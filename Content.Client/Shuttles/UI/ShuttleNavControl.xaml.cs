...
using Content.Shared._Exodus.Territory; // Exodus - TerritoryMarker
...
public partial class ShuttleNavControl : BaseShuttleControl // Mono
{
    ...
    private EntityQuery<TerritoryMarkerComponent> _territoryQuery; // Exodus - TerritoryMarker
    // Exodus - SafeZone - End
    ...
    public ShuttleNavControl(float minRange, float maxRange, float range) : base(minRange, maxRange, range) // Mono
    {
        ...
        _territoryQuery = EntManager.GetEntityQuery<TerritoryMarkerComponent>(); // Exodus - TerritoryMarker
        // Exodus - SafeZone - End
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        DrawTerritoryMarkers(handle, worldToView, ourGridId); // Exodus - TerritoryMarker
    ...
    // Exodus - TerritoryMarker - Start
    private void DrawTerritoryMarkers(DrawingHandleScreen handle, Matrix3x2 worldToView, EntityUid? ourGridUid)
    {
        if (ourGridUid is not { } ourGrid)
            return;

        var ourMapId = _transform.GetMapCoordinates(ourGrid).MapId;

        var query = EntManager.AllEntityQueryEnumerator<TerritoryMarkerComponent, TransformComponent>();
        while (query.MoveNext(out _, out var territory, out var xform))
        {
            if (xform.MapID != ourMapId)
                continue;

            var worldPos = _transform.GetWorldPosition(xform);
            var posInView = Vector2.Transform(worldPos, worldToView);
            var screenRadius = territory.Radius * MinimapScale;

            handle.DrawCircle(posInView, screenRadius, territory.FillColor, filled: true);
            handle.DrawCircle(posInView, screenRadius, territory.BorderColor, filled: false);
        }
    }
    // Exodus - TerritoryMarker - End
